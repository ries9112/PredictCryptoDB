...
2. Using the [docker container available to download on Metabase's website](https://hub.docker.com/r/metabase/metabase/), I created a server-type environment in AWS that acts as an interface with Metabase.

3. The AWS instance that runs the Metabase environment can be connected to through the https secured website [predictcryptodb.com](https://predictcryptodb.com) using the set of credentials that you created after clicking on the invitation from the e-mail you received. If you need an invite to create a set of credentials, e-mail me at riccardo.esclapon@colorado.edu.
